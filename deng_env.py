...
class Deng(Environment):
    ...
    def __init__(self, folder, steps, train_data, test_data, test_starts, **kwargs):
        ...
        Load mkt_data and set action and observation space
        """
        ...
        self.data_mean = np.std(self.data[1:] - self.data[:-1])  # Get the mean of the mkt_data to normalise the input
    ...
    def step(self, action):
        ...
        3) get reward
        return: the new state, reward and if the mkt_data are done
        """

        # record actions for calc pnl later
        ...
        # If the are still more mkt_data to process
